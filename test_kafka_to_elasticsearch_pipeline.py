...
def elasticsearch_data(sch, pipeline, cluster,elasticsearch):
    ...
    admin_client = KafkaAdminClient(bootstrap_servers=argsv[11].cluster-server, client_id='test',security_protocol="PLAINTEXT")
    #es1 = Elasticsearch([{"host":"172.28.0.4","port":9200}])
    es1 = Elasticsearch([{"host":argsv[9],"port":9200}])
